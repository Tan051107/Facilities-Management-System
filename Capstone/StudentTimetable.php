<?php 
    ...
    $GetIntakeQuery = "SELECT * FROM student_details WHERE StudentID = '$userId'";
    $GetIntakeResult = mysqli_query($connection, $GetIntakeQuery);

   if ($GetIntakeResult && mysqli_num_rows($GetIntakeResult) > 0) {
        $GetIntake = mysqli_fetch_assoc($GetIntakeResult);
        $IntakeCode = $GetIntake['IntakeCode'];
    } else {
        echo "No student found with the provided StudentID.";
    }
    ...
    $selectedDay = $_POST['selectedDay'] ?? $currentDay;
    ...
    $query = "(
        ...
        WHERE st.IntakeCode = ? AND ts1.Day = ?
    )
    UNION ALL
    (
        SELECT 
        ...
        JOIN lecturer_details ld ON lt.LecturerID = ld.LecturerID
        JOIN module_information mi ON st.ModuleCode = mi.ModuleCode
        WHERE st.IntakeCode = ? AND ts2.Day = ?
    )
    UNION ALL
    (
        SELECT 
        ...
        JOIN lecturer_details ld ON lt.LecturerID = ld.LecturerID
        JOIN module_information mi ON st.ModuleCode = mi.ModuleCode
        WHERE st.IntakeCode = ? AND ts3.Day = ?
    )
    UNION ALL
    (
        SELECT 
        ...
        JOIN lecturer_details ld ON lt.LecturerID = ld.LecturerID
        JOIN module_information mi ON st.ModuleCode = mi.ModuleCode
        WHERE st.IntakeCode = ? AND ts4.Day = ?
    )";

    $stmt = mysqli_prepare($connection, $query);
    if ($stmt) {
        mysqli_stmt_bind_param($stmt, 'ssssssss', 
            $IntakeCode, $selectedDay,
            $IntakeCode, $selectedDay,
            $IntakeCode, $selectedDay,
            $IntakeCode, $selectedDay
        );
    ...
    $cancelquery = "SELECT cts.ModuleCode AS ModuleID, cts.OriginDate AS OriginalDate FROM changetimeslot cts
                    ...
                    WHERE st.IntakeCode = '".$escapedIntakeCode."' AND cts.OriginDate = '".$escapedDate."'";
    ...
    if ($stmt) {
        ...
        echo "Failed to prepare replacement statement: " . mysqli_error($connection);
        exit; 
    }
...
<head>
    ...
    <link rel="stylesheet" href="StudentTimetable.css?v=<?php echo time(); ?>">
...
<body class="lightmode">
        ...
        <div class="left-group">
            ...
            <nav class="navbar hideOnMobile">
                <?php if ($userType === 'Admin'): ?>
                    <a href="IntakeTable.php">Create Timetable</a>
    ...
    <div class="UserTimetable">
        ...
        <div class="timetable-container" id="timetable">
            ...
            ?>
                ...
                if (empty($data) && empty($data1) && empty($holidayRecord)){ ?>
    ...
    <script src="StudentTimetable.js?v=<?php echo time(); ?>"></script>
